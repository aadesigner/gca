/**
 * Pick VIN with real exterior ThreeSixty + real InteriorImageRetriever pano.
 */
...
// also confirm the sentra we fixed
const sentra = await c.query(`
  SELECT photo_group, count(*) n, min(left(source_url,100)) sample
...
const fleet = await c.query(`
  SELECT count(*) FILTER (WHERE status='running')::int running,
         count(*) FILTER (WHERE status='running' AND updated_at < NOW()-interval '15 minutes')::int quiet15
