...
const r = await c.query(`
  SELECT p.internal_name, j.id, j.status, j.items_discovered, j.items_processed, j.items_failed, j.vins_new,
         round(extract(epoch from (NOW()-j.updated_at))/60)::int quiet_m
  FROM collection_jobs j JOIN providers p ON p.id=j.provider_id
  WHERE j.status='running' AND p.internal_name = ANY($1::text[])
  ORDER BY 1
...
`);
console.log("finnNew10m", g.rows[0]);
await c.end();
