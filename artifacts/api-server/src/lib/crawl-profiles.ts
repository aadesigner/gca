...
export const CRAWL_PROFILES: Record<string, CrawlProfile> = {
  ...
  auctionauto: {
    extraction: "api-json",
    summary:
      "auctionauto.org: Korea/USA sharded by make→model (API 10k window). VIN-only persist; sold uses saleDate; photos filtered by VIN.",
    delayMs: 400,
    concurrency: 6,
    retryCount: 3,
    skipRecentHours: 12,
  ...
  autopartner: {
    extraction: "html-vin-detail",
    summary:
      "cars.autopartner.by: Copart/IAAI/Encar. Search pages → /v/{VIN}; gallery only frames containing this VIN.",
    delayMs: 300,
    concurrency: 4,
    retryCount: 3,
    skipRecentHours: 12,
  ...
  nfsauto: {
    extraction: "html-vin-detail",
    summary:
      "nfsauto.by: Korea+China load-more API (~130k). Lot HTML VIN; Encar photos filtered by lot id.",
    delayMs: 250,
    concurrency: 6,
    retryCount: 3,
    skipRecentHours: 12,
  ...
  autovit: {
    extraction: "html-vin-rare",
    summary:
      "Autovit.ro (OLX Romania). Specs/photos free; VIN mostly login/captcha — persist when plaintext in description.",
    delayMs: 1400,
    concurrency: 2,
    retryCount: 3,
    skipRecentHours: 12,
  ...
  beforward: {
    extraction: "html-vin-detail",
    summary:
      "BE FORWARD (beforward.jp) JP export marketplace. Make-sharded stocklist; Chassis No. (ISO VIN or JP frame); USD FOB; image-cdn gallery in site order (original preferred). Slow crawl + 429 backoff (local IPs rate-limit easily).",
    delayMs: 1600,
    concurrency: 1,
    retryCount: 4,
    skipRecentHours: 0,
  ...
  japaneseusedcars: {
    extraction: "html-vin-detail",
    summary:
      "japaneseusedcars.com fixed-price catalogs. Chassis usually masked (****) — low VIN persist rate; AJES photos when present.",
    delayMs: 800,
    concurrency: 2,
    retryCount: 2,
    skipRecentHours: 12,
  ...
  syarah: {
    extraction: "html-vin-detail",
    summary:
      "Syarah.com SA marketplace. FULL_PAGE_DATA specs + ordered CDN gallery (SAR). Public pages omit VIN — fleet-skipped until VIN yield.",
    delayMs: 500,
    concurrency: 3,
    retryCount: 3,
    skipRecentHours: 12,
  ...
  import_motor: {
    extraction: "html-vin-detail",
    summary:
      "Import Motor CDP buyer-locations country crawl (priority destinations first; Korean list cards first within each page when fullCrawl). Skips VIN+mileage+photo-complete lots; brand mode only shallow (~page 6 wall).",
    delayMs: 85,
    concurrency: 10,
    retryCount: 5,
    skipRecentHours: 0,
